...
sfVector2f get_rotated_point(sfVector2f pt, sfVector2f center, float angle)
{
    sfVector2f rotated_point;

    angle = angle * (M_PI / 180);
    rotated_point.x = cos(angle) * (pt.x - center.x)
                        - sin(angle) * (pt.y - center.y) + center.x;
    rotated_point.y = sin(angle) * (pt.x - center.x)
                        + cos(angle) * (pt.y - center.y) + center.y;
    return (rotated_point);
}

sfFloatRect get_bounding_box_of_rotated(sfFloatRect const htbox, float angle)
{
    sfVector2f *rotated_corners = get_rotated_corners(htbox, angle);
